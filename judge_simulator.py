...
LLM_MODEL = "gpt-4.1-mini"  # <-- Optional: specify model or leave empty for default
...
def main():
    ...
    # Resolve API key and model from environment if not set in config
    global LLM_API_KEY, LLM_MODEL, BOT_URL
    if not LLM_API_KEY:
        LLM_API_KEY = os.environ.get("OPENAI_API_KEY", "")
    if not LLM_MODEL:
        LLM_MODEL = os.environ.get("OPENAI_MODEL", "")
    bot_port = os.environ.get("PORT", "")
    if bot_port and BOT_URL == "http://localhost:8080":
